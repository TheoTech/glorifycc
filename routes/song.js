  ...
  .get(function(req, res, next) {
    if (song.copyright === copyrightTypes.private) {
      if (!req.isAuthenticated()) {
        ...
        song.contributor === req.user.username ||
        passportFunction.isAdmin(req)
      ) {
    ...
    function findSong() {
      Song.find({
        ...
        .exec(function(err, translations) {
          ...
          var leftSong =
            translations.find(translation => translation.lang.code === left) ||
            {};
          var rightSong =
            translations.find(translation => translation.lang.code === right) ||
            {};
          var rightSongExists = !_.isEmpty(rightSong);
          var leftSongExists = !_.isEmpty(leftSong);
          if (!leftSongExists) {
            res.redirect(
              '/song/' + song._id + '?left=' + song.lang.code + '&right='
            );
          }
          if (req.isAuthenticated()) {
            Playlist.find(
